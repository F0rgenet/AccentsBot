...
from utils.accents_quiz.models import TestQuestion, EgeQuestion, Word, Category
...
class AccentsQuizManager(metaclass=SingletonMeta):
    ...
    async def get_test_questions(self, count: int, categories: Literal["all"] | list[str] = "all") -> list[
        TestQuestion]:
        questions = []
        while len(questions) < count:
