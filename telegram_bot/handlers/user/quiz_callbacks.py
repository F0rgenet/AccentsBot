...
@router.callback_query(F.data == "select_quiz")
...
async def quiz_ege_keyboard_changed(callback: CallbackQuery, callback_data: QuizEgeKeyboardCallback, state: FSMContext):
    ...
    keyboard = await keyboards.generate_for_ege(data["questions"][data["current_question_index"] - 1],
...
@router.callback_query(F.data == "done_quiz")
